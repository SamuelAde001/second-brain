...
- Every agent runs in the main session on Opus 5.5, the project default model in `.claude/settings.json` (Samuel, 2026-09-23). Bulk extraction is done by scripts, not read into context. A subagent or a cheaper model only when Samuel asks for one on a given task. Cloud routines run on Sonnet, except the night plan (Opus, 2026-09-25).
