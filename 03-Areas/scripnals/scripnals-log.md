...
- 2026-09-22 — **Guide library v1 drafted** (→ [[03-Areas/scripnals/guides/guide-library|Guide library]]). Samuel asked for real research on what performs on Instagram, content types, script formats, hooks and what makes a hook good, with feedback that stays natural to the user, sorted by content type, niche and format, readable by AIs and easy to put in a database. Research and sources → [[03-Areas/scripnals/guide-library-research|Guide library research]]. 66 tagged entries, exported to `guides.json` + `vocab.json` by `00-System/scripts/build_scripnals_guides.py`. The spec moved to **v1.1 (Proposed)**: niche + format inputs and tag matching. PDF: `assets/scripnals-script-buddy-ai-workflow-v1.1.pdf`. Waiting for Samuel's yes (open question 80) and questions 75–79.
- 2026-09-22 — Samuel's review of the first draft (→ [[03-Areas/scripnals/scripnals-decisions|Decisions]]): guides are advice, not rules; no niche question, no format picker; keyword and send CTAs allowed; no dialect names; no examples from his content; make it token-light. Done: niches and formats folded into one note each (51 entries), all entries rewritten terse (largest request ~3,500 → ~2,050 words), master prompt says the guides are advice. Spec v1.1 PDF rebuilt with the panel as it was. Open: the score bands and the hook names.
