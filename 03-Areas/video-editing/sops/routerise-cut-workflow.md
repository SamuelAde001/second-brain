...
4. **Remove bad takes** — judged against the clips now sitting on the timeline, not against raw transcript runs.
   ...
   ### What Samuel still had to fix on Route Rise #3 (2026-09-25): read before step 4
   He took Cut v6 (11:02) to his final (9:55), removing 1,612 frames: 10 whole clips I left unflagged (5 retakes, 2 noise, 3 stray bits), 14 of my Orange/Pink flags, and 30 trims inside kept clips (uhms, pauses, in-clip retakes). None of my 54 cuts was restored. The misses have four causes:
   1. **Subtitle-to-clip mapping by midpoint is wrong.** Resolve's subtitle lines run across clip boundaries, so I put words on the wrong clip. Clean whole-clip retakes then looked like in-clip retakes or unique lines: "If you've ever built a prospect list" (a whole earlier take), "And putting all that context by writing" (a whole earlier take), "Connecting the role…" (a false start), the second attempt at the Claude prompt. **Get text per clip before judging.** Use the per-clip transcription (step 4, "Per-clip transcription"), with each empty clip joined to the next. Never judge from a whole-timeline subtitle track.
   2. **I flagged instead of deciding.** In every flagged case Samuel kept the later clean take, the same rule used for whole clips. **Apply "last clean complete take" inside clips too, and decide.** Colour a clip only when the content exists nowhere else. Samuel called the markers and colours "clutter" and removed them all.
   3. **Noise rule too strict.** "No 50 ms window above −40 dB" missed breaths, clicks and "uh"s: clips under ~1 s with no transcribed word, where the sentences around them read complete without them. Cut those. (Short lead-in words exist too, so check with the per-clip text joined to the next clip.)
   4. **Uhms and pauses inside clips.** Resolve's transcription drops disfluencies (see step 3), and the silence pass leaves voiced "uh"s and short pauses. Samuel trimmed 2–65 frames at heads, tails and middles of 30 clips. That needs a pass that can see them: voiced audio not covered by any transcribed word, from a word-timed transcript that keeps fillers. Tooling not built yet.
