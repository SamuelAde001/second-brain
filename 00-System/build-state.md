...
- **Next action:** two tracks. (1) **Scripnals guide library: drafted 2026-09-22; review, test and send by 2026-09-27** → `03-Areas/scripnals/guides/guide-library.md`. (2) Phase 4: **finance agent next** (brief: [[03-Areas/finances/finance-agent-plan|finance agent plan]]), then personal-life (TickTick), then content.
...
- **Guide library session done (2026-09-22, session 5).** Research → `03-Areas/scripnals/guide-library-research.md` (Instagram's own rules, Mosseri, Meta engagement bait, Kallaway, Socialinsider; every source coded). **66 tagged entries** in `03-Areas/scripnals/guides/`, overview `guides/guide-library.md`, export `guides/_export/guides.json` + `vocab.json` from `00-System/scripts/build_scripnals_guides.py`. Spec moved to **v1.1, Proposed** (niche + format inputs, tag matching), PDF `assets/scripnals-script-buddy-ai-workflow-v1.1.pdf`. **Waiting on Samuel:** yes/no on v1.1 (Q80) before he sends the PDF, then Q75–79. **Next Scripnals step:** Samuel reviews the 17 core/action/content-type entries, test on 3–4 real drafts (`--select … --prompt`), send guides by 2026-09-27.
