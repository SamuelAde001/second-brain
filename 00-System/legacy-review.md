...
**Status values:** `unreviewed` → `kept` / `changed` / `dropped`, each with the date it was decided.
...
Each was written by the weekly review with ledger evidence. They are claims about how Samuel fails, and he confirms or kills them.
...
| # | Item | Where | Domain | Status |
|---|------|-------|--------|--------|
| R1 | Morning brief — sets the day's commitment from TickTick | `.claude/skills/brief/` | rituals | unreviewed |
| R2 | Midday checkpoint (3pm) — "catches a dead day while there's still day left to fix it" | `.claude/skills/midday/` | rituals | unreviewed |
...
Cite by link; do not copy into active notes.

| File | What it holds | Domain |
|------|---------------|--------|
| `context/ledger.md`, `context/ledger-notes/` | Daily ledger and the written-up weeks, Aug–Sep 2026, including "the dark stretch, 4–14 Sep" | me |
| `context/money-ledger.md` | Savings movements and withdrawals with reasons | finances |
| `context/decisions.md` | 586 lines of dated decisions, each marked BUILT | all |
| `context/memory.md` | Working notes from the engine's own operation | system |
| `context/audience.md` | Audience and course-pricing thinking, incl. the open beta price | personal-brand, academy |
...
Scanned for the never-store list in AGENTS.md §6: **no credentials, account numbers, BVN, NIN, tokens or passwords found.** `SHEETS_TOKEN` appears only as a variable name and a `__SHEETS_TOKEN__` placeholder. The long digit strings in `context/ticktick.md` and `PA.md` are TickTick project IDs, not financial identifiers. `.env` was never copied.
