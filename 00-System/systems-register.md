...
| `00-System/scripts/session_start_sync.py` | Fetches GitHub, fast-forwards `main` (only on `main`, only fast-forward), and lists unmerged `claude/*` cloud-session branches. Never merges, deletes or resets; always exits 0 | automatically, by the SessionStart hook |
