...
| `00-System/scripts/budget_sheet.py` | Rebuilds the "Money" Google Sheet (tabs Overview and Ledger) from the money ledger, the obligations plan and the goals, over Google's official Sheets API as a service account. `preview` prints it without touching the sheet, `doctor` checks the connection, `snapshot YYYY-MM` freezes a month's plan at its close | after any ledger or plan change; `snapshot` at every month close |
